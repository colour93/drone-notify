...
fn load_or_init_config(path: &Path) -> Result<Config> {
    ...
    if cfg.base_url.as_deref().map(str::trim).map(|s| s.is_empty()).unwrap_or(true) {
        cfg.base_url = Some(
    ...
    if cfg.default_namespace.as_deref().map(str::trim).map(|s| s.is_empty()).unwrap_or(true) {
    ...
    if cfg.default_repo.as_deref().map(str::trim).map(|s| s.is_empty()).unwrap_or(true) {
    ...
    if cfg.default_branch.as_deref().map(str::trim).map(|s| s.is_empty()).unwrap_or(true) {
    ...
    if cfg.token.as_deref().map(str::trim).map(|s| s.is_empty()).unwrap_or(true) {
        cfg.token = Some(
            Input::with_theme(&theme)
...
async fn fetch_user(client: &Client, config: &Config) -> Result<UserResponse> {
    ...
    Ok(res.json::<UserResponse>().await.context("解析用户响应失败")?)
}
